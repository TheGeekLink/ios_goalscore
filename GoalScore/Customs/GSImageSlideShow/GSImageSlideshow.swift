...
public protocol ImageSlideshowDelegate: class {
    ...
    ///   - page: new page
    @objc optional func imageSlideshow(_ imageSlideshow: GSImageSlideshow, didChangeCurrentPageTo page: Int)
    
    /// Tells the delegate that the slideshow will begin dragging
    ///
    /// - Parameter imageSlideshow: image slideshow instance
    @objc optional func imageSlideshowWillBeginDragging(_ imageSlideshow: GSImageSlideshow)
    
    /// Tells the delegate that the slideshow did end decelerating
    ///
    /// - Parameter imageSlideshow: image slideshow instance
    @objc optional func imageSlideshowDidEndDecelerating(_ imageSlideshow: GSImageSlideshow)
}
...
@objcMembers
open class GSImageSlideshow: UIView {
    ...
    open var currentSlideshowItem: GSImageSlideshowItem? {
    ...
    open fileprivate(set) var slideshowItems = [GSImageSlideshowItem]()
    ...
    func reloadScrollView() {
        ...
        for image in scrollViewImages {
            let item = GSImageSlideshowItem(image: image, zoomEnabled: zoomEnabled, activityIndicator: activityIndicator?.create(), maximumScale: maximumScale)
    ...
    fileprivate func setTimerIfNeeded() {
        if slideshowInterval > 0 && scrollViewImages.count > 1 && slideshowTimer == nil {
            slideshowTimer = Timer.scheduledTimer(timeInterval: slideshowInterval, target: self, selector: #selector(GSImageSlideshow.slideshowTick(_:)), userInfo: nil, repeats: true)
        }
    }
    ...
    func setupImageSlideShow(controller: GSBaseViewController, imageSources: [ImageSource]) {
        
        self.slideshowInterval = 2.0
        self.pageIndicatorPosition = .init(horizontal: .center, vertical: .customBottom(padding: 0))
        self.contentScaleMode = UIView.ContentMode.scaleAspectFill
        ...
        let recognizer = CustomTapGesture(target: self, action: #selector(self.didTap(sender:)))
    ...
    @objc func didTap(sender: CustomTapGesture) {
        if let controller = sender.controller {
            let fullScreenController = self.presentFullScreenController(from: controller)
            // set the activity indicator for full screen controller (skipping the line will show no activity indicator)
            fullScreenController.slideshow.activityIndicator = DefaultActivityIndicator(style: .white, color: .blue)
        }
    }
    
}

extension GSImageSlideshow: UIScrollViewDelegate {
